      ...
      <?php
          ...
          try
          {
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0)
            {
              ...
              try 
              {
                ...
                //----------------------------------------------------------------------------------------
                // 1. DB Connect include
                //----------------------------------------------------------------------------------------
                try {
                    $dbh = new PDO('mysql:host=adinfo.co.kr;port=30600;dbname=ADINFO', "dbfactory", "db11!!");
                } catch (PDOException $e) {
                    die();
                }

                $query = "
                  INSERT INTO AD_CARD_PAYMENT
                  (
                        UPDATE_DT
                      , TID
                      , PAY_METHOD
                ...
                $stmt->bindParam(':TID'          , $resultMap["tid"]);
                $stmt->bindParam(':PAY_METHOD'   , $resultMap["payMethod"]);
                $stmt->bindParam(':RESULT_CODE'  , $resultMap["resultCode"]);
                $stmt->bindParam(':RESULT_MSG'   , $resultMap["resultMsg"]);
                $stmt->bindParam(':TOTAL_AMT'    , $resultMap["TotPrice"]);
                $stmt->bindParam(':MOID'         , $resultMap["MOID"]);
                $stmt->bindParam(':APPL_DATE'    , $resultMap["applDate"]);
                $stmt->bindParam(':APPL_TIME'    , $resultMap["applTime"]);
                $stmt->bindParam(':CARD_NO'      , $resultMap["CARD_Num"]);
                $stmt->bindParam(':buyerEmail'   , $resultMap["buyerEmail"]);
          ...
          if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){  //결제보안 추가 2016-05-18
            ...
            ******************************************************************************/
                  ...
                  echo "<tr><th class='td01'><p>결제 여부</p></th>";
