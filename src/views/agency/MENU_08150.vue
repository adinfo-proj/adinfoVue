		...
		<div id="menu08101">
			<div class="campSearch">
				<div class="campSearchSub">
					<select v-model="campaignStatusCode" @change="campaignListChange(campaignStatusCode, 1, true)" disabled>
				...
				<table class="campTable">
					<thead>
						<tr>
							...
							<th class="campPay"   	  >광고주명</th>
							<th class="marketer"  	  >SMS 수신 여부</th>
					...
					<tbody>
						<tr v-for="(campaignFullData, index) in campaignFullDataObj" :key="index">
							...
							<td class="campNameData" >{{ campaignFullData.name }}</td>
							<td class="campCode"     >{{ campaignFullData.landCount }} 개</td>
							<td class="campPay"      >{{ campaignFullData.adName }} </td>
              <td class="marketer"     v-if="campaignFullData.smsYn == 'Y'">예</td>
              <td class="marketer"     v-else                              >아니오</td>
...
<style scoped>
	...
	#menu08101 .campDataBox td.campNameData {
		text-align: left;
	}
